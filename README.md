...
| 聊天页 | `#chat` | 三轮问答、F/M 性别选择 | 四步进度提示；依次选择回答；选择性别后才显示“选择我的标签” |
...
项目根目录的 `biome.json` 是 IDE 插件与命令行共享的唯一代码规范来源。提交前建议依次执行：
...
当前推荐使用腾讯云 CloudBase 静态网站托管。它不需要维护裸服务器，可以直接发布 Vite 的 `dist/`，适合本次需要快速生成二维码、主要面向中国大陆访问者的临时展示。
...
CloudBase 默认域名更适合开发与短期测试，可能存在访问频率限制。长期正式使用时，应准备完成 ICP 备案的自定义域名并绑定到托管服务。
...
- CloudBase 默认域名适合临时展示，不作为长期正式生产域名方案。
